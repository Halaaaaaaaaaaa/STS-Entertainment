...
-- 고객센터 테이블
-- 컬럼명: 문의번호, 회원id, 회원이름, 회원메일, 문의종류, 문의제목, 문의내용, 문의상태, 작성일, 답변
create table cs (
    cseq number primary key,
    id varchar2(20) not null,
        constraint fk_cs_id foreign key(id) references member(id),
    name varchar2(20) not null,
    email varchar2(40) not null,
    coption varchar2(15) not null,
    ctitle varchar2(50) not null,
    ccontent varchar2(500) not null,
    cstatus varchar2(15) default '답변대기',
    cregdate Date default TO_DATE(sysdate, 'YYYY-MM-DD'),
    reply varchar2(500)
);
